...
class MiniImagenet(Dataset):
    ...
    def __init__(self, root, mode, batchsz, n_way, k_shot, k_query, resize, startidx=0):
        ...
        print('查看字典img2label:',self.img2label)
    ...
    def create_batch(self, batchsz):
        ...
        for b in range(batchsz):  # for each batch
            ...
            np.random.shuffle(selected_cls) # 打乱顺序
            support_x = []
            query_x = []
    ...
    def __getitem__(self, index):
        ...
        # print('self.support_x_batch[index]:',self.support_x_batch[index])
        # sublist=self.support_x_batch[index][0]
        # item=sublist[0]
        # print('int(item[:3]):',int(item[:3]))
        # print('self.img2label[int(item[:3]):',self.img2label[str(int(item[:3]))])

        support_y = np.array(
            [self.img2label[str(int(item[:3]))]  # IITD
...
if __name__ == '__main__':
    ...
    # from tensorboardX import SummaryWriter # tensorboardX安装不了，但是ptorch1.1之后的版本中有SummaryWriter方法
    ...
    # mini = MiniImagenet('E:/Documents/Matlab_work/DataBase/miniImagenet/', mode='train',
